...
CommandManager.add('modlogChannel', {description: 'Change the channel for the mod logs', category: 'Settings', admin: true}, async (message, content) => {
    ...
    if (content == '') {
        ...
        var msgEmbed = new MessageEmbed().setColor(CommandManager.successColor).setTitle(`Mod log channel has been removed`);
    ...
    var channel = await message.guild.channels.fetch(channelId).catch(() => {
        var msgEmbed = new MessageEmbed().setColor(CommandManager.failColor).setTitle(`No channel found`);
        message.channel.send({embeds: [msgEmbed]});
        Console.log(`Failed to set mod log channel`, message.guild.id);
    });
    if (!channel) return;
    functions.setModLog(channel.id, message.guild);
    var msgEmbed = new MessageEmbed().setColor(CommandManager.successColor).setTitle(`The mod log channel has been set to`).setDescription(`${channel}`);
    message.channel.send({embeds: [msgEmbed]});
...
CommandManager.add('logChannel', {description: 'Change the channel for the general logs', category: 'Settings', admin: true}, async (message, content) => {
    ...
    if (content == '') {
        ...
        functions.setModLog(null, message.guild);
        var msgEmbed = new MessageEmbed().setColor(CommandManager.successColor).setDescription(`Mod log channel has been removed`);
        message.channel.send({embeds: [msgEmbed]});
    ...
    var channelId = content.replace('<#', '').replace('>', '');
    var channel = await message.guild.channels.fetch(channelId).catch(() => {
        var msgEmbed = new MessageEmbed().setColor(CommandManager.failColor).setDescription(`No channel found`);
        message.channel.send({embeds: [msgEmbed]});
        Console.log(`Failed to set general log channel`, message.guild.id);
    });
    if (!channel) return;
    functions.setGeneralLog(channel.id, message.guild);
    var msgEmbed = new MessageEmbed().setColor(CommandManager.successColor).setDescription(`The general log channel has been set to ${channel}`);
    message.channel.send({embeds: [msgEmbed]});
...
CommandManager.add('modRole', {description: 'Add or remove a role as moderator', category: 'Settings', admin: true}, async (/** @type {import('discord.js').Message} */ message, content) => {
    ...
    if (params[0] == 'add') {
        ...
        if (!role) {
            var msgEmbed = new MessageEmbed().setColor(CommandManager.failColor).setDescription(`Couldn't find role`);
            message.channel.send({embeds: [msgEmbed]});
        ...
        if (result) {
            var msgEmbed = new MessageEmbed().setColor(CommandManager.successColor).setDescription(`${role} has been added as a moderator role`);
            message.channel.send({embeds: [msgEmbed]});
            Console.log(`${role.name} has been added as a moderator role`, message.guild.id);
        } else {
            var msgEmbed = new MessageEmbed().setColor(CommandManager.failColor).setDescription(`${role} is already a moderator role`);
            message.channel.send({embeds: [msgEmbed]});
        ...
        if (result) {
            var msgEmbed = new MessageEmbed().setColor(CommandManager.successColor).setDescription(`${role} has been removed from the moderator roles`);
            message.channel.send({embeds: [msgEmbed]});
            Console.log(`${role.name} has been removed from the moderator roles`, message.guild.id);
        } else {
            var msgEmbed = new MessageEmbed().setColor(CommandManager.failColor).setDescription(`${role} is not a moderator role`);
            message.channel.send({embeds: [msgEmbed]});
        ...
        var msgEmbed = new MessageEmbed().setColor(CommandManager.failColor).setTitle(`Moderator roles has been cleared`);
        message.channel.send({embeds: [msgEmbed]});
        ...
        var msgEmbed = new MessageEmbed().setColor(CommandManager.failColor).setTitle(`Moderator roles`);
